...
function getFnScopeInfo(file, scope) {
  ...
  if (type == "Program") {
    ...
    var _ret = (function () {
      ...
      try {
        walk.simple(file.ast, {
          ...
          VariableDeclarator: function VariableDeclarator(node) {
            if (node.init == scope.originNode) {
              throw new found(node.id);
            }
          }
        });
      } catch (e) {
      ...
      name = (scope.fnType ? scope.fnType.name : scope.originNode.name) || "<anonymous>";
      start = scope.fnType ? scope.fnType.originNode.start : scope.originNode.start;
...
function storeCaller(query, file, callers) {
  ...
  return function (node, scope) {
    ...
    if (caller) {
      ...
      if (start != caller.start && start != query.end) {
...
function processProperty(server, file, prop, parent, level, resType, resolvedTypes) {
  ...
  if (prop.propertyName != "prototype") {
    var nodeType = getPropType(prop.types);
        ...
        if (shouldResolve) {
          ...
          if (type.originNode && type.originNode.sourceFile == file && (type.originNode.type == "FunctionExpression" || type.originNode.type == "FunctionDeclaration")) {
            ...
            if (scope) {
              ...
              if (scope.fnType) {
                if (scope.originNode) {
                  child.blockStart = scope.originNode.start;
                  child.blockEnd = scope.originNode.end;
                }
                gather(server, file, scope, child ? child : parent, level, "scope", resolvedTypes);
              }
            }
...
function getPropType(types) {
  var result = "?";
  var filteredTypes = [];
  if (types.length > 0) {
    for (var typeNr in types) {
      var type = types[typeNr];
      if (filteredTypes.indexOf(type.toString()) == -1) {
        filteredTypes.push(type.toString());
      }
    }
    result = filteredTypes.join('|');
  }
  return result;
}
    ...
    for (var elementNr in body) {
      ...
      for (var fnNr in fns) {
        ...
        if (!fnExpr.name) {
          ...
          if (fnExpr.scope) {
            if (fnExpr.scope.originNode) {
              child.blockStart = fnExpr.scope.originNode.start;
              child.blockEnd = fnExpr.scope.originNode.end;
            }
