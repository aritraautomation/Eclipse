                ...
                mostSpecific.visit({
                    ...
                    visitText: function (ast) {
                        ...
                        if (mostSpecific) {
                            ...
                            var visitor = new ExpressionDocVisitor(templateInfo, templatePosition_1, name, kind, undefined, function () { return language_services_1.getExpressionScope(dinfo_1, templatePath, false); });
...
function attributeValueCompletions(info, position, attr) {
    ...
    if (mostSpecific) {
        ...
        if (!visitor.result || !visitor.result.length) {
            ...
            var widerPath_1 = utils_1.findTemplateAstAt(info.templateAst, position, /* allowWidening */ /* allowWidening */ true);
            if (widerPath_1.tail) {
...
function attributeValueDocumentation(info, position, attr, name, type) {
    ...
    if (mostSpecific) {
        ...
        var visitor = new ExpressionDocVisitor(info, position, name, type, attr, function () { return language_services_1.getExpressionScope(dinfo_2, path, false); });
        mostSpecific.visit(visitor, null);
        if (!visitor.result || !visitor.result.length) {
            // Try allwoing widening the path
            var widerPath_2 = utils_1.findTemplateAstAt(info.templateAst, position, /* allowWidening */ /* allowWidening */ true);
            if (widerPath_2.tail) {
                var widerVisitor = new ExpressionDocVisitor(info, position, name, type, attr, function () { return language_services_1.getExpressionScope(dinfo_2, widerPath_2, false); });
...
var ExpressionVisitor = (function (_super) {
    ...
    ExpressionVisitor.prototype.visitElement = function (ast) {
        ...
        if (this.attr && utils_1.getSelectors(this.info) && this.attr.name.startsWith(TEMPLATE_ATTR_PREFIX)) {
            ...
            var binding = bindings.find(function (binding) { return utils_1.inSpan(valueRelativePosition_1, binding.span, /* exclusive */ /* exclusive */ true); }) ||
