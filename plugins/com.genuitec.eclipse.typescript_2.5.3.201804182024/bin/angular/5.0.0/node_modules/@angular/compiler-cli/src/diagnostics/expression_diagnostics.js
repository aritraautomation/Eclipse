...
function getTemplateExpressionDiagnostics(info, getAttributeInfosForElement, validateAttribute) {
    ...
    }, getAttributeInfosForElement, validateAttribute);
    ...
    function processReferences(references) {
        var _loop_1 = function (reference) {
            ...
            result.push({
                ...
                get definition() { return getDefinitionOf(info, reference, "reference", reference.name); },
...
function getDefinitionOf(info, ast, kind, name) {
    if (info.fileName) {
...
function getVarDeclarations(info, path) {
    ...
    for (var _i = 0, _a = list.reverse(); _i < _a.length; _i++) {
        ...
        var _loop_2 = function (variable) {
            ...
            var context = current_1.directives.map(function (d) { return info.query.getTemplateContext(d.directive.type.reference); })
            ...
            if (context) {
                ...
                if (value) {
                    ...
                    if (kind === symbols_1.BuiltinType.Any || kind == symbols_1.BuiltinType.Unbound) {
                        ...
                        // better.
                        type = refinedVariableType(type, info, current_1, result);
                    }
                }
            }
            if (!type) {
            ...
            result.push({
                ...
                kind: 'variable',
                type: type,
                get definition() { return getDefinitionOf(info, variable, 'var', name_1); },
                get documentation() { return getDocumentationOf(info, variable, 'var', name_1); }
            });
        };
        for (var _b = 0, _c = current_1.variables; _b < _c.length; _b++) {
            var variable = _c[_b];
            _loop_2(variable);
        }
    }
...
function refinedVariableType(type, info, templateElement, templateVariables) {
    ...
    if (ngForDirective) {
        ...
        if (ngForOfBinding) {
            var bindingType = new expression_type_1.AstType(info.query.mergeSymbolTable([info.members, info.query.createSymbolTable(templateVariables)]), info.query, {}).getType(ngForOfBinding.value);
            if (bindingType) {
...
var ExpressionDiagnosticsVisitor = (function (_super) {
    ...
    function ExpressionDiagnosticsVisitor(info, getExpressionScope, getAttributeInfosForElement, validateAttribute) {
        var _this = _super.call(this) || this;
        _this.info = info;
        _this.getExpressionScope = getExpressionScope;
        _this.getAttributeInfosForElement = getAttributeInfosForElement;
        _this.validateAttribute = validateAttribute;
        _this.diagnostics = [];
    ...
    ExpressionDiagnosticsVisitor.prototype.visitAttr = function (ast) {
        ...
        var msg = this.validateAttribute(attrName);
        var infos = this.getAttributeInfosForElement(element.name, element);
        var caseNotMatch = false;
        var found = infos.find(function (info) {
            return info.name === attrName
