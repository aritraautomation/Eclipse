...
Evaluator = (function () {
    ...
    /**
       ...
       *   table.
       */
    Evaluator.prototype.isFoldable = /**
       * Returns true if the expression represented by `node` can be folded into a literal expression.
       *
       * For example, a literal is always foldable. This means that literal expressions such as `1.2`
       * `"Some value"` `true` `false` are foldable.
    ...
    /**
       ...
       * tree are folded. For example, a node representing `1 + 2` is folded into `3`.
       */
    Evaluator.prototype.evaluateNode = /**
       * Produce a JSON serialiable object representing `node`. The foldable values in the expression
       * tree are folded. For example, a node representing `1 + 2` is folded into `3`.
       */
    function (node, preferReference) {
        var _this = this;
        var t = this;
        var error;
        ...
        switch (node.kind) {
                    ...
                    switch (child.kind) {
                        ...
                        case ts.SyntaxKind.PropertyAssignment:
                            ...
                            var propertyValue = isPropertyAssignment(assignment) ?
                                _this.evaluateNode(assignment.initializer, /* preferReference */ /* preferReference */ true) :
                                resolveName(propertyName, /* preferReference */ /* preferReference */ true);
            ...
            case ts.SyntaxKind.ArrayLiteralExpression:
                var arr_1 = [];
                ts.forEachChild(node, function (child) {
                    var value = _this.evaluateNode(child, /* preferReference */ /* preferReference */ true);
                    // Check for error
