...
_NodeEmitterVisitor = (function () {
    ...
    _NodeEmitterVisitor.prototype.getReexports = function () {
        return Array.from(this._reexports.entries())
            ...
            return ts.createExportDeclaration(undefined, undefined, ts.createNamedExports(reexports.map(function (_a) {
    ...
    _NodeEmitterVisitor.prototype.getImports = function () {
        return Array.from(this._importsWithPrefixes.entries())
            ...
            return ts.createImportDeclaration(undefined, undefined, /* importClause */ ts.createImportClause(
            /* name */ undefined, ts.createNamespaceImport(ts.createIdentifier(prefix))), /* moduleSpecifier */ createLiteral(namespace));
        });
    };
    ...
    // StatementVisitor
    // StatementVisitor
    _NodeEmitterVisitor.prototype.visitDeclareVarStmt = 
    // StatementVisitor
    function (stmt) {
        ...
        var varDeclList = ts.createVariableDeclarationList([ts.createVariableDeclaration(ts.createIdentifier(stmt.name), undefined, (stmt.value && stmt.value.visitExpression(this, null)) || undefined)]);
        if (stmt.hasModifier(compiler_1.StmtModifier.Exported)) {
            ...
            var tsVarStmt = this.record(stmt, ts.createVariableStatement(/* modifiers */ /* modifiers */ [], varDeclList));
            var exportStmt = this.record(stmt, ts.createExportDeclaration(undefined, /*modifiers*/ undefined, ts.createNamedExports([ts.createExportSpecifier(stmt.name, stmt.name)])));
    ...
    _NodeEmitterVisitor.prototype.visitDeclareFunctionStmt = function (stmt, context) {
        return this.record(stmt, ts.createFunctionDeclaration(undefined, this.getModifiers(stmt), undefined, stmt.name, /* typeParameters */ undefined, stmt.params.map(function (p) {
            return ts.createParameter(undefined, /* modifiers */ undefined, undefined, p.name);
    ...
    _NodeEmitterVisitor.prototype.visitDeclareClassStmt = function (stmt) {
        ...
        var fields = stmt.fields.map(function (field) {
            return ts.createProperty(undefined, /* modifiers */ undefined, field.name, undefined, undefined, ts.createNull());
        });
        var getters = stmt.getters.map(function (getter) {
            return ts.createGetAccessor(undefined, /* modifiers */ undefined, getter.name, /* parameters */ /* parameters */ [], undefined, _this._visitStatements(getter.body));
        });
        var constructor = (stmt.constructorMethod && [ts.createConstructor(undefined, undefined, /* parameters */ stmt.constructorMethod.params.map(function (p) {
                return ts.createParameter(undefined, undefined, undefined, p.name);
        ...
        var methods = stmt.methods.filter(function (method) { return method.name; })
            .map(function (method) {
            return ts.createMethod(undefined, /* modifiers */ undefined, undefined, (method.name) /* guarded by filter */, undefined, /* typeParameters */ undefined, method.params.map(function (p) {
                return ts.createParameter(undefined, /* modifiers */ undefined, undefined, p.name);
            }), undefined, _this._visitStatements(method.body));
        });
        return this.record(stmt, ts.createClassDeclaration(undefined, modifiers, stmt.name, /* typeParameters*/ undefined, stmt.parent && [ts.createHeritageClause(ts.SyntaxKind.ExtendsKeyword, [stmt.parent.visitExpression(this, null)])] ||
    ...
    _NodeEmitterVisitor.prototype.visitTryCatchStmt = function (stmt) {
        return this.record(stmt, ts.createTry(this._visitStatements(stmt.bodyStmts), ts.createCatchClause(CATCH_ERROR_NAME, this._visitStatementsPrefix([ts.createVariableStatement(undefined, [ts.createVariableDeclaration(CATCH_STACK_NAME, /* type */ undefined, ts.createPropertyAccess(ts.createIdentifier(CATCH_ERROR_NAME), ts.createIdentifier(CATCH_STACK_NAME)))])], stmt.catchStmts)), undefined));
    };
    ...
    // ExpressionVisitor
    // ExpressionVisitor
    _NodeEmitterVisitor.prototype.visitReadVarExpr = 
    // ExpressionVisitor
    function (expr) {
        switch (expr.builtin) {
    ...
    _NodeEmitterVisitor.prototype.visitInvokeMethodExpr = function (expr) {
        ...
        return this.record(expr, ts.createCall(ts.createPropertyAccess(expr.receiver.visitExpression(this, null), methodName), undefined, expr.args.map(function (arg) { return arg.visitExpression(_this, null); })));
    };
    ...
    _NodeEmitterVisitor.prototype.visitFunctionExpr = function (expr) {
        return this.record(expr, ts.createFunctionExpression(undefined, /* astriskToken */ undefined, /* name */ undefined, undefined, expr.params.map(function (p) {
            return ts.createParameter(undefined, /* modifiers */ undefined, undefined, p.name);
    ...
    _NodeEmitterVisitor.prototype.visitLiteralMapExpr = function (expr) {
        var _this = this;
        return this.record(expr, ts.createObjectLiteral(expr.entries.map(function (entry) {
            return ts.createPropertyAssignment(entry.quoted || !_VALID_IDENTIFIER_RE.test(entry.key) ?
