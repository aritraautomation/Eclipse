...
var ClosureRewriter = (function (_super) {
    ...
    ClosureRewriter.prototype.getJSDoc = function (node) {
        ...
        if (!parsed)
            return null;
        if (parsed.warnings) {
            var start = node.getFullStart() + startPosition;
...
function emitWithTsickle(program, host, tsHost, tsOptions, targetSourceFile, writeFile, cancellationToken, emitOnlyDtsFiles, customTransformers) {
    ...
    if (customTransformers) {
        if (customTransformers.beforeTsickle) {
            beforeTsTransformers.unshift.apply(beforeTsTransformers, __spread(customTransformers.beforeTsickle));
        }
        if (customTransformers.beforeTs) {
            beforeTsTransformers.push.apply(beforeTsTransformers, __spread(customTransformers.beforeTs));
        }
        if (customTransformers.afterTs) {
            afterTsTransformers.push.apply(afterTsTransformers, __spread(customTransformers.afterTs));
        }
    }
    var tsTransformers = transformer_util_1.createCustomTransformers({
