...
function bindDirectiveAfterContentLifecycleCallbacks(directiveMeta, directiveInstance, compileElement) {
    ...
    if (lifecycleHooks.indexOf(private_import_core_1.LifecycleHooks.AfterContentInit) !== -1) {
    ...
    if (lifecycleHooks.indexOf(private_import_core_1.LifecycleHooks.AfterContentChecked) !== -1) {
...
function bindDirectiveAfterViewLifecycleCallbacks(directiveMeta, directiveInstance, compileElement) {
    ...
    if (lifecycleHooks.indexOf(private_import_core_1.LifecycleHooks.AfterViewInit) !== -1) {
    ...
    if (lifecycleHooks.indexOf(private_import_core_1.LifecycleHooks.AfterViewChecked) !== -1) {
...
function bindDirectiveWrapperLifecycleCallbacks(dir, directiveWrapperIntance, compileElement) {
    compileElement.view.destroyMethod.addStmts(directive_wrapper_compiler_1.DirectiveWrapperExpressions.ngOnDestroy(dir.directive, directiveWrapperIntance));
    compileElement.view.detachMethod.addStmts(directive_wrapper_compiler_1.DirectiveWrapperExpressions.ngOnDetach(dir.hostProperties, directiveWrapperIntance, o.THIS_EXPR, compileElement.compViewExpr || o.THIS_EXPR, compileElement.renderNode));
}
exports.bindDirectiveWrapperLifecycleCallbacks = bindDirectiveWrapperLifecycleCallbacks;
function bindInjectableDestroyLifecycleCallbacks(provider, providerInstance, compileElement) {
    var onDestroyMethod = compileElement.view.destroyMethod;
    onDestroyMethod.resetDebugInfo(compileElement.nodeIndex, compileElement.sourceAst);
    if (provider.providerType !== template_ast_1.ProviderAstType.Directive &&
        provider.providerType !== template_ast_1.ProviderAstType.Component &&
        provider.lifecycleHooks.indexOf(private_import_core_1.LifecycleHooks.OnDestroy) !== -1) {
...
function bindPipeDestroyLifecycleCallbacks(pipeMeta, pipeInstance, view) {
    ...
    if (pipeMeta.type.lifecycleHooks.indexOf(private_import_core_1.LifecycleHooks.OnDestroy) !== -1) {
