...
var class_builder_1 = require('./output/class_builder');
...
exports.DirectiveWrapperCompileResult = DirectiveWrapperCompileResult;
...
var DirectiveWrapperCompiler = (function () {
    ...
    DirectiveWrapperCompiler.dirWrapperClassName = function (id) {
        return "Wrapper_" + compile_metadata_1.identifierName(id);
    };
    ...
    DirectiveWrapperCompiler = __decorate([
        ...
        __metadata('design:paramtypes', [config_1.CompilerConfig, parser_1.Parser, element_schema_registry_1.ElementSchemaRegistry, private_import_core_1.Console])
...
var DirectiveWrapperBuilder = (function () {
    function DirectiveWrapperBuilder(compilerConfig, dirMeta) {
        ...
        this.genChanges = dirLifecycleHooks.indexOf(private_import_core_1.LifecycleHooks.OnChanges) !== -1 ||
            this.compilerConfig.logBindingUpdate;
        this.ngOnChanges = dirLifecycleHooks.indexOf(private_import_core_1.LifecycleHooks.OnChanges) !== -1;
        this.ngOnInit = dirLifecycleHooks.indexOf(private_import_core_1.LifecycleHooks.OnInit) !== -1;
        this.ngDoCheck = dirLifecycleHooks.indexOf(private_import_core_1.LifecycleHooks.DoCheck) !== -1;
        this.ngOnDestroy = dirLifecycleHooks.indexOf(private_import_core_1.LifecycleHooks.OnDestroy) !== -1;
        if (this.ngOnDestroy) {
    ...
    DirectiveWrapperBuilder.prototype.build = function () {
        ...
        var methods = [
            new o.ClassMethod('ngOnDetach', [
                new o.FnParam(VIEW_VAR.name, o.importType(identifiers_1.createIdentifier(identifiers_1.Identifiers.AppView), [o.DYNAMIC_TYPE])),
                new o.FnParam(COMPONENT_VIEW_VAR.name, o.importType(identifiers_1.createIdentifier(identifiers_1.Identifiers.AppView), [o.DYNAMIC_TYPE])),
        ...
        ctorStmts.push(o.THIS_EXPR.prop(CONTEXT_FIELD_NAME)
            ...
            .toStmt());
        return class_builder_1.createClassStmt({
...
function addNgDoCheckMethod(builder) {
    ...
    if (builder.genChanges) {
        ...
        if (builder.compilerConfig.logBindingUpdate) {
            onChangesStmts.push(o.importExpr(identifiers_1.createIdentifier(identifiers_1.Identifiers.setBindingDebugInfoForChanges))
...
function addCheckInputMethod(input, builder) {
    var field = binding_util_1.createCheckBindingField(builder);
    ...
    if (builder.genChanges) {
        onChangeStatements.push(o.THIS_EXPR.prop(CHANGES_FIELD_NAME)
            .key(o.literal(input))
            .set(o.importExpr(identifiers_1.createIdentifier(identifiers_1.Identifiers.SimpleChange))
            .instantiate([field.expression, CURR_VALUE_VAR]))
            .toStmt());
    }
    var methodBody = binding_util_1.createCheckBindingStmt({ currValExpr: CURR_VALUE_VAR, forceUpdate: FORCE_UPDATE_VAR, stmts: [] }, field.expression, THROW_ON_CHANGE_VAR, onChangeStatements);
...
function addCheckHostMethod(hostProps, hostEvents, builder) {
    ...
    hostProps.forEach(function (hostProp, hostPropIdx) {
        var field = binding_util_1.createCheckBindingField(builder);
        var evalResult = expression_converter_1.convertPropertyBinding(builder, null, o.THIS_EXPR.prop(CONTEXT_FIELD_NAME), hostProp.value, field.bindingId);
        if (!evalResult) {
            return;
        }
        ...
        if (hostProp.needsRuntimeSecurityContext) {
            ...
            methodParams.push(new o.FnParam(securityContextExpr.name, o.importType(identifiers_1.createIdentifier(identifiers_1.Identifiers.SecurityContext))));
        }
        var checkBindingStmts;
        if (hostProp.isAnimation) {
            var _a = render_util_1.triggerAnimation(VIEW_VAR, COMPONENT_VIEW_VAR, hostProp, hostEvents, o.THIS_EXPR.prop(EVENT_HANDLER_FIELD_NAME)
                .or(o.importExpr(identifiers_1.createIdentifier(identifiers_1.Identifiers.noop))), RENDER_EL_VAR, evalResult.currValExpr, field.expression), updateStmts = _a.updateStmts, detachStmts = _a.detachStmts;
            checkBindingStmts = updateStmts;
            (_b = builder.detachStmts).push.apply(_b, detachStmts);
        }
        else {
            checkBindingStmts = render_util_1.writeToRenderer(VIEW_VAR, hostProp, RENDER_EL_VAR, evalResult.currValExpr, builder.compilerConfig.logBindingUpdate, securityContextExpr);
        }
        stmts.push.apply(stmts, binding_util_1.createCheckBindingStmt(evalResult, field.expression, THROW_ON_CHANGE_VAR, checkBindingStmts));
        var _b;
    });
...
function addHandleEventMethod(hostListeners, builder) {
    ...
    hostListeners.forEach(function (hostListener, eventIdx) {
        var evalResult = expression_converter_1.convertActionBinding(builder, null, o.THIS_EXPR.prop(CONTEXT_FIELD_NAME), hostListener.handler, "sub_" + eventIdx);
    ...
    builder.methods.push(new o.ClassMethod('handleEvent', [
        ...
        new o.FnParam(expression_converter_1.EventHandlerVars.event.name, o.DYNAMIC_TYPE)
...
function parseHostBindings(dirMeta, exprParser, schemaRegistry) {
    var errors = [];
    var parser = new binding_parser_1.BindingParser(exprParser, interpolation_config_1.DEFAULT_INTERPOLATION_CONFIG, schemaRegistry, [], errors);
    var moduleUrl = compile_metadata_1.identifierModuleUrl(dirMeta.type);
    var sourceFileName = moduleUrl ?
        "in Directive " + compile_metadata_1.identifierName(dirMeta.type) + " in " + moduleUrl :
        "in Directive " + compile_metadata_1.identifierName(dirMeta.type);
...
function reportParseErrors(parseErrors, console) {
    var warnings = parseErrors.filter(function (error) { return error.level === parse_util_1.ParseErrorLevel.WARNING; });
    var errors = parseErrors.filter(function (error) { return error.level === parse_util_1.ParseErrorLevel.FATAL; });
...
var DirectiveWrapperExpressions = (function () {
    ...
    DirectiveWrapperExpressions.ngOnDestroy = function (dir, dirWrapper) {
        if (dir.type.lifecycleHooks.indexOf(private_import_core_1.LifecycleHooks.OnDestroy) !== -1 ||
...
exports.DirectiveWrapperExpressions = DirectiveWrapperExpressions;
