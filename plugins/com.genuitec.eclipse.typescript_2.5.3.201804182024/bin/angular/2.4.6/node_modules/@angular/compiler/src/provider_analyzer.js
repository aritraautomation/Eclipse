...
var ProviderViewContext = (function () {
    function ProviderViewContext(component, sourceSpan) {
        ...
        component.viewProviders.forEach(function (provider) {
            if (lang_1.isBlank(_this.viewProviders.get(compile_metadata_1.tokenReference(provider.token)))) {
...
var ProviderElementContext = (function () {
    function ProviderElementContext(viewContext, _parent, _isViewRoot, _directiveAsts, attrs, refs, _sourceSpan) {
        ...
        if (lang_1.isPresent(queriedTokens.get(identifiers_1.resolveIdentifier(identifiers_1.Identifiers.ViewContainerRef)))) {
            this._hasViewContainer = true;
        }
        // create the providers that we know are eager first
        Array.from(this._allProviders.values()).forEach(function (provider) {
            var eager = provider.eager || lang_1.isPresent(queriedTokens.get(compile_metadata_1.tokenReference(provider.token)));
            if (eager) {
    ...
    ProviderElementContext.prototype._addQueryReadsTo = function (token, queryReadTokens) {
        this._getQueriesFor(token).forEach(function (query) {
            ...
            if (lang_1.isBlank(queryReadTokens.get(compile_metadata_1.tokenReference(queryReadToken)))) {
                queryReadTokens.set(compile_metadata_1.tokenReference(queryReadToken), true);
            }
        });
    };
    ...
    ProviderElementContext.prototype._getOrCreateLocalProvider = function (requestingProviderType, token, eager) {
        ...
        if (lang_1.isPresent(this._seenProviders.get(compile_metadata_1.tokenReference(token)))) {
    ...
    ProviderElementContext.prototype._getLocalDependency = function (requestingProviderType, dep, eager) {
        ...
        if (lang_1.isPresent(dep.token)) {
            ...
            if ((requestingProviderType === template_ast_1.ProviderAstType.Directive ||
                ...
                if (compile_metadata_1.tokenReference(dep.token) === identifiers_1.resolveIdentifier(identifiers_1.Identifiers.Renderer) ||
                    compile_metadata_1.tokenReference(dep.token) === identifiers_1.resolveIdentifier(identifiers_1.Identifiers.ElementRef) ||
                    compile_metadata_1.tokenReference(dep.token) === identifiers_1.resolveIdentifier(identifiers_1.Identifiers.ChangeDetectorRef) ||
                    compile_metadata_1.tokenReference(dep.token) === identifiers_1.resolveIdentifier(identifiers_1.Identifiers.TemplateRef)) {
                    return dep;
                }
                if (compile_metadata_1.tokenReference(dep.token) === identifiers_1.resolveIdentifier(identifiers_1.Identifiers.ViewContainerRef)) {
                    this._hasViewContainer = true;
                }
            }
            // access the injector
            if (compile_metadata_1.tokenReference(dep.token) === identifiers_1.resolveIdentifier(identifiers_1.Identifiers.Injector)) {
                return dep;
            }
            // access providers
        ...
        if (dep.isSelf) {
            ...
            if (!result) {
                if (!dep.isHost || this.viewContext.component.isHost ||
                    ...
                    lang_1.isPresent(this.viewContext.viewProviders.get(compile_metadata_1.tokenReference(dep.token)))) {
                    result = dep;
                }
                else {
...
var NgModuleProviderAnalyzer = (function () {
    ...
    NgModuleProviderAnalyzer.prototype._getDependency = function (dep, eager, requestorSourceSpan) {
        ...
        if (!dep.isSkipSelf && lang_1.isPresent(dep.token)) {
            // access the injector
            if (compile_metadata_1.tokenReference(dep.token) === identifiers_1.resolveIdentifier(identifiers_1.Identifiers.Injector) ||
                compile_metadata_1.tokenReference(dep.token) === identifiers_1.resolveIdentifier(identifiers_1.Identifiers.ComponentFactoryResolver)) {
                foundLocal = true;
            }
