...
function locateSymbol(info) {
    ...
    var path = new template_path_1.TemplateAstPath(info.templateAst, templatePosition);
    if (path.tail) {
        ...
        var attributeValueSymbol_1 = function (ast, inEvent) {
            ...
            if (attribute) {
                if (utils_1.inSpan(templatePosition, utils_1.spanOf(attribute.valueSpan))) {
                    var scope = expressions_1.getExpressionScope(info, path, inEvent);
        ...
        path.tail.visit({
            ...
            visitReference: function (ast) {
                symbol_1 = ast.value ? info.template.query.getTypeSymbol(compiler_1.tokenReference(ast.value)) : null;
                span_1 = utils_1.spanOf(ast);
            },
            ...
            visitBoundText: function (ast) {
                ...
                if (utils_1.inSpan(expressionPosition, ast.value.span)) {
                    var scope = expressions_1.getExpressionScope(info, path, /* includeEvent */ false);
...
function findAttribute(info) {
    var templatePosition = info.position - info.template.span.start;
    var path = new html_path_1.HtmlAstPath(info.htmlAst, templatePosition);
    return path.first(ast_1.Attribute);
}
function findInputBinding(info, path, binding) {
    var element = path.first(template_ast_1.ElementAst);
    if (element) {
