...
var TypeScriptFileValidator = (function () {
    function TypeScriptFileValidator(source, declarations, analyzedModules, host, basePath, getDirectiveModuleUri, getTemplateClassFromStaticSymbol) {
        ...
        this.result = [];
        this.urlResolver = compiler_1.createOfflineCompileUrlResolver();
    }
    ...
    TypeScriptFileValidator.prototype.validateComponent = function (directive, decorator) {
        ...
        if (modules.length > 1) {
            var modulesStr_1 = ""; //${stringify(oldModule)} and ${stringify(moduleType)}
            modules.forEach(function (module, index) {
                ...
                modulesStr_1 += util_1.stringify(module.type.reference);
            });
            var typeStr = util_1.stringify(directive.type.reference);
            this.addDiagnostic(decorator, null, false, (directive.isComponent ? "Component" : "Directive") + " " + typeStr + " is part of the declarations of modules " + modulesStr_1 + ". " +
                ("Please consider moving " + typeStr + " to a higher module that imports " + modulesStr_1 + ". ") +
                ("You can also create a new NgModule that exports and includes " + typeStr + " then import that NgModule in " + modulesStr_1), types_1.DiagnosticKind.Warning);
        }
    };
    TypeScriptFileValidator.prototype.validateTemplateAttributes = function (directive, decorator) {
        ...
        else if (isPresent(url = directive.template.templateUrl)) {
        ...
        else if (isPresent(template = directive.template.template)) {
